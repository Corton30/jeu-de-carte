...
//route pour recuperer toutes les parties dispo
app.post('/gameDataRecovery', async (req, res) => {
    try {
        const partieDispo = await Game.findAll();
        res.json(partieDispo);
    } catch (error) {
        console.error("erreur !");
        res.status(500).json({ error: "errreur serveur !" });
    }
});
